	...
	public Card chooseCard() {
		...
		for (Card card : hand) {
			if (Board.getBoard().isEmpty()) {
				...
				boolean matchLastCard = card.getFace().equals(Board.getBoard().get(Board.getBoard().size() - 1).getFace());
				boolean isJack = card.getFace().equals("J");

				if (matchLastCard || isJack) {
					botCard = card;
					break;
				} else {
					botCard = hand.get(randomIndex);
...
class ExpertBot implements Player {
	...
	public Card chooseCard() {
		...
		int[] frequencyOfCards = new int[13];
		for (Card card : cardsPlayed) {
			int index = Arrays.asList(faces).indexOf(card.getFace());
			if (index != -1) {
				frequencyOfCards[index]++;
			}
		}
		int mostFrequentCard = 0;
		for (int i = 0; i < frequencyOfCards.length; i++) {
			if (frequencyOfCards[i] > frequencyOfCards[mostFrequentCard]) {
				mostFrequentCard = i;
			}
		}

		// Choose a card from the bot's hand.
		Card botCard = null;
		for (Card card : hand) {
			if (Board.getBoard().isEmpty()) {
				if (card.getFace().equals(faces[mostFrequentCard])) {
					botCard = card;
					break;
				}
			} else {
				boolean matchLastCard = card.getFace().equals(Board.getBoard().get(Board.getBoard().size() - 1).getFace());
				boolean matchMostRepeatedValue = card.getFace().equals(faces[mostFrequentCard]);
				boolean isJack = card.getFace().equals("J");

				if (matchLastCard || isJack || matchMostRepeatedValue) {
					botCard = card;
					break;
				}
			}
